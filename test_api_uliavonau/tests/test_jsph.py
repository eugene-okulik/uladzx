...
def test_update_object_using_put(create_object_endpoint, put_update_object_endpoint, delete_object_endpoint):
    data = {
    ...
    response = put_update_object_endpoint.put_edit_object(object_id, updated_data).json()
    put_update_object_endpoint.check_response_status_is_200()
    put_update_object_endpoint.check_response_name_is_updated(response['name'])
    put_update_object_endpoint.check_response_data_is_updated(response['data'])

    delete_object_endpoint.delete_object(object_id)
    delete_object_endpoint.check_response_status_is_200()


def test_update_object_using_patch(create_object_endpoint, patch_update_object_endpoint, delete_object_endpoint):
    data = {
        "name": "10.10.2024 object",
        "data": {"color": "red", "size": "large", "userId": 1}
    }
    ...
    updated_data = {
        "name": "10.10.2024 object_UPD_using_PATCH",
    }
    patch_update_object_endpoint.patch_edit_object(object_id, updated_data).json()
    patch_update_object_endpoint.check_response_status_is_200()
    patch_update_object_endpoint.check_response_name_is_updated(updated_data['name'])

    delete_object_endpoint.delete_object(object_id)
    delete_object_endpoint.check_response_status_is_200()


def test_delete_created_object(create_object_endpoint, delete_object_endpoint):
    data = {
        "name": "10.10.2024 object",
        "data": {"color": "red", "size": "large", "userId": 1}
    }
